"use client";

import { useState } from "react";
import { useRoom } from "@/app/hooks/useRoom";
import type { Song } from "@/components/context/RoomContext";

interface VoteError {
  message: string;
  id: string;
}

export default function Queue() {
  const { queue, voteSong } = useRoom();
  const [voteError, setVoteError] = useState<VoteError | null>(null);
  const [optimisticVotes, setOptimisticVotes] = useState<
    Record<string, { votes: number; userVote: "up" | "down" | null }>
  >({});

  const getSongVoteState = (song: Song) => {
    if (optimisticVotes[song.id]) {
      return optimisticVotes[song.id];
    }
    return { votes: song.votes, userVote: song.userVote ?? null };
  };

  const handleVote = (songId: string, direction: "up" | "down") => {
    try {
      setVoteError(null);

      const song = queue.find((s) => s.id === songId);
      if (!song) return;

      const currentState = getSongVoteState(song);
      const isCurrentlyUpvoted = currentState.userVote === "up";
      const isCurrentlyDownvoted = currentState.userVote === "down";

      let newVotes = currentState.votes;
      let newUserVote: "up" | "down" | null = direction;

      if (direction === "up") {
        if (isCurrentlyUpvoted) {
          newUserVote = null;
          newVotes = Math.max(0, currentState.votes - 1);
        } else if (isCurrentlyDownvoted) {
          newVotes = currentState.votes + 1;
        } else {
          newVotes = currentState.votes + 1;
        }
      } else if (direction === "down") {
        if (isCurrentlyDownvoted) {
          newUserVote = null;
          newVotes = currentState.votes + 1;
        } else if (isCurrentlyUpvoted) {
          newVotes = Math.max(0, currentState.votes - 1);
        } else {
          newVotes = Math.max(0, currentState.votes - 1);
        }
      }

      setOptimisticVotes((prev) => ({
        ...prev,
        [songId]: { votes: newVotes, userVote: newUserVote },
      }));

      voteSong(songId, direction);
    } catch (error) {
      setOptimisticVotes((prev) => {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const { [songId]: _, ...rest } = prev;
        return rest;
      });
      setVoteError({
        id: songId,
        message: error instanceof Error ? error.message : "Cannot vote",
      });
      setTimeout(() => setVoteError(null), 3000);
    }
  };

  return (
    <div className="flex flex-col gap-3">
      {/* Error toast */}
      {voteError && (
        <div className="px-4 py-3 rounded-xl border border-red-500/30 bg-red-500/10 text-red-400 font-mono text-[11px] animate-pulse">
          ⚠️ {voteError.message}
        </div>
      )}

      {/* Header */}
      <div className="flex items-center justify-between">
        <span className="font-mono text-[10px] tracking-[0.15em] text-[#4A4038] uppercase">
          Up Next
        </span>
        <span className="font-mono text-[10px] tracking-[0.08em] text-[#C8892A]">
          {queue.length} {queue.length === 1 ? "track" : "tracks"}
        </span>
      </div>

      {/* Empty state */}
      {queue.length === 0 && (
        <div className="flex flex-col items-center justify-center h-28 gap-2 rounded-xl border border-[#E8A030]/8 bg-[#161310]">
          <span className="text-2xl opacity-30">♪</span>
          <p className="font-mono text-[11px] tracking-[0.1em] text-[#4A4038] uppercase">
            No songs in queue yet
          </p>
        </div>
      )}

      {/* Queue list */}
      {queue.length > 0 && (
        <div className="flex flex-col gap-2">
          {queue.map((song, index) => {
            const voteState = getSongVoteState(song);
            return (
              <div
                key={song.id}
                className="flex items-center gap-3 px-4 py-3 rounded-xl border border-[#E8A030]/[0.07] bg-[#161310] hover:border-[#E8A030]/20 hover:bg-[#1E1A16] transition-colors group overflow-hidden"
              >
                {/* Rank */}
                <div className="w-7 flex items-center justify-center shrink-0">
                  {index === 0 ? (
                    <span className="font-mono text-[9px] tracking-[0.1em] text-[#C8892A] bg-[#E8A030]/10 border border-[#E8A030]/20 px-[6px] py-[3px] rounded">
                      TOP
                    </span>
                  ) : (
                    <span className="font-mono text-[13px] text-[#4A4038] group-hover:text-[#6B5F50] transition-colors">
                      {String(index + 1).padStart(2, "0")}
                    </span>
                  )}
                </div>

                {/* Song info */}
                <div className="flex-1 min-w-0">
                  <p className="font-serif text-[13px] font-medium text-[#C8B89A] truncate leading-snug">
                    {song.title}
                  </p>
                  <p className="font-mono text-[10px] text-[#4A4038] mt-[3px] truncate">
                    @{song.addedBy}
                  </p>
                </div>

                {/* Vote buttons */}
                <div className="flex flex-col items-center gap-[3px] shrink-0">
                  <button
                    onClick={() => handleVote(song.id, "up")}
                    disabled={voteError?.id === song.id}
                    className={`w-6 h-5 flex items-center justify-center rounded text-[10px] border transition-colors cursor-pointer
                                        ${
                                          voteState.userVote === "up"
                                            ? "text-[#C8892A] border-[#E8A030]/40 bg-[#E8A030]/10"
                                            : "text-[#4A4038] border-[#E8A030]/[0.07] hover:text-[#C8892A] hover:border-[#E8A030]/30"
                                        } ${voteError?.id === song.id ? "opacity-50 cursor-not-allowed" : ""}`}
                  >
                    ▲
                  </button>

                  <span
                    className={`font-mono text-[11px] font-medium tabular-nums leading-none
                                      ${voteState.votes > 0 ? "text-[#C8892A]" : voteState.votes < 0 ? "text-[#8B4A3A]" : "text-[#6B5F50]"}`}
                  >
                    {voteState.votes}
                  </span>

                  <button
                    onClick={() => handleVote(song.id, "down")}
                    disabled={voteError?.id === song.id}
                    className={`w-6 h-5 flex items-center justify-center rounded text-[10px] border transition-colors cursor-pointer
                                        ${
                                          voteState.userVote === "down"
                                            ? "text-[#8B4A3A] border-[#8B4A3A]/40 bg-[#8B4A3A]/10"
                                            : "text-[#4A4038] border-[#E8A030]/[0.07] hover:text-[#8B4A3A] hover:border-[#8B4A3A]/30"
                                        } ${voteError?.id === song.id ? "opacity-50 cursor-not-allowed" : ""}`}
                  >
                    ▼
                  </button>
                </div>
              </div>
            );
          })}
        </div>
      )}
    </div>
  );
}
